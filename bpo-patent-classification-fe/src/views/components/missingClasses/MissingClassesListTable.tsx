...
const MissingClassesListTable = ({ total, records, blockTable, onPageOrOrderChange, group }) => {
            ...
            {records.map((row, index) => (
              <TableRow hover key={row.symbol}>
                ...
                <OptionTableCell>
                  <TableButton
                    type={"view"}
                    to={`/classifier/missing-classes/view/${row.symbol.replace("/", "-")}/${row.edition}`}
                  />
                </OptionTableCell>
              </TableRow>
            ))}
...
export default MissingClassesListTable;
