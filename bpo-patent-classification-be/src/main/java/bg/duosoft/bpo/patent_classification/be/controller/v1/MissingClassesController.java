...
@Slf4j
@RestController
@RequestMapping("/api/v1/missing-classes")
@RequiredArgsConstructor
public class MissingClassesController {
    ...
    public Page<IpcTableRowDTO> searchData(IpcClassesFilterDTO filter) {
        if (!StringUtils.hasText(activityToken.getCurrentProcess())) {
            List<IpcTableRowDTO> results = ipcService.searchMissingClasses(filter);
            return new PageImpl<>(results, PageRequest.of(filter.getPage(), filter.getPageSize()), ipcService.getMissingClassesCount(filter));
        }
        return null;
    }
}
